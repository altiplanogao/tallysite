 ...
 */
define(['react', 'react-dom', "jquery", "underscore", "i18n!nls/menutext"],
    function (React, ReactDOM, $, _, menutext) {
    ...
    var BreadcrumbSeg = React.createClass({
        render: function () {
            ...
            var a = (entry.url == "") ? <span >{menutext[entry.name]}</span> : <a href={entry.url}>{menutext[entry.name]}</a>;
            return (<li>{a}</li>);
        }
    });
    ...
    var Breadcrumb = React.createClass({
        entryRouting:function(menuPath, menuData){
            ...
            menuPath.forEach(function(node){
                var match = _.find(current.entries, function(entry){ return entry.key == node; });
